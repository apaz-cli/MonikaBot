// Due to a change in Discord's JSON responses, it was necessary to change a 
// single line in the getPresenceFromJSON() method to handle a case where the
// game type is 4, and default to a playing message. It would perhaps be better 
// to have an actual implementation for the fourth type, but seeing as I never 
// plan to use presences to do anything, I feel confident in 
// ~Aaron Pazdera
...
public class DiscordUtils {
	...
	private static IPresence getPresenceFromJSON(GameObject game, String status) {
		return new Presence(
				...
				game == null || game.type == 4 ? ActivityType.PLAYING : ActivityType.values()[game.type]);
	}
